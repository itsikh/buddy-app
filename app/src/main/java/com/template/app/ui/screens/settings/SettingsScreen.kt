                ...
                Card(modifier = Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(12.dp)) {
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            FilterChip(
                                selected = driveUiState.aiDefaultProvider == com.template.app.AppConfig.AI_PROVIDER_GEMINI,
                                onClick  = { viewModel.setAiDefaultProvider(com.template.app.AppConfig.AI_PROVIDER_GEMINI) },
                                label    = { Text("Gemini (מומלץ)") }
                            )
                            FilterChip(
                                selected = driveUiState.aiDefaultProvider == com.template.app.AppConfig.AI_PROVIDER_CLAUDE,
                                onClick  = { viewModel.setAiDefaultProvider(com.template.app.AppConfig.AI_PROVIDER_CLAUDE) },
                                label    = { Text("Claude") }
                            )
                        }
                    }
